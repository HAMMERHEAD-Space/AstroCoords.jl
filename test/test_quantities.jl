@testset "Orbital Quantities" begin
    ...
    @testset "Orbital Period" begin
        ...
        T = orbitalPeriod(a, μ)
        @test T > 0.0
        @test T ≈ 2π / √(μ / a^3) rtol=1e-14

        # Test with Keplerian elements
        kep = Keplerian(a, 0.01, 0.1, 0.2, 0.3, 0.4)
        T_kep = orbitalPeriod(kep, μ)
        @test T_kep ≈ T rtol=1e-14
    end
    ...
    @testset "orbitalNRG(a, μ) Direct Computation" begin
        ...
        @test nrg_large ≈ -μ / (2.0 * a_large) rtol=1e-14
    end
    ...
    @testset "Type Promotion in Quantities" begin
    ...
    @testset "Edge Cases: a→∞ (Escape Energy)" begin
    ...
    @testset "Angular Momentum Vector" begin
        ...
        @test length(h_vec) == 3
        @test h_vec[3] ≈ 7000.0 * 7.5 rtol=1e-14  # h = r × v, z-component

        # Test with AstroCoord
        cart = Cartesian(u)
        h_vec_coord = angularMomentumVector(cart, μ)
        @test h_vec_coord ≈ h_vec rtol=1e-14
    end
    ...
    @testset "Angular Momentum Quantity" begin
        ...
        @test h > 0.0
        @test h ≈ 7000.0 * √(μ/7000.0) rtol=1e-14

        # Test with AstroCoord
        cart = Cartesian(u)
        h_coord = angularMomentumQuantity(cart, μ)
        @test h_coord ≈ h rtol=1e-14
    end
    ...
    @testset "Vis-Viva Equation Verification" begin
        ...
        @test v_squared_actual ≈ v_squared_visviva rtol=1e-14

        # Verify energy from vis-viva
        nrg_visviva = v_squared_actual/2.0 - μ/r_mag
        nrg_direct = orbitalNRG(a, μ)
        @test nrg_visviva ≈ nrg_direct rtol=1e-14
    end
end
