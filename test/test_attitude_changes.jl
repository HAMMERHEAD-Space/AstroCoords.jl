...
@testset "Attitude Transformations" begin
    @testset "EP2MRP - Identity Rotation" begin
        ...
        @test σ[1] ≈ 0.0 atol=1e-10
        @test σ[2] ≈ 0.0 atol=1e-10
        @test σ[3] ≈ 0.0 atol=1e-10
    end
    ...
    @testset "EP2MRP - Small Angle Rotation" begin
        ...
        β = SVector{4,Float64}(cos(θ/2), sin(θ/2)*axis[1], sin(θ/2)*axis[2], sin(θ/2)*axis[3])
        σ = AstroCoords.EP2MRP(β)
        
        # For small angles, σ ≈ (θ/4) * axis
        expected_mag = tan(θ/4)
        @test norm(σ) ≈ expected_mag atol=1e-9
    end
    ...
    @testset "MRP2EP - Identity" begin
        ...
        @test β[1] ≈ 1.0 atol=1e-10  # β0 = 1 for identity
        @test β[2] ≈ 0.0 atol=1e-10
        @test β[3] ≈ 0.0 atol=1e-10
        @test β[4] ≈ 0.0 atol=1e-10
    end
    ...
    @testset "MRP2EP - 90° Rotation" begin
        ...
        @test β[1] ≈ cos(θ/2) atol=1e-10
        @test β[2] ≈ sin(θ/2) atol=1e-10
        @test β[3] ≈ 0.0 atol=1e-10
        @test β[4] ≈ 0.0 atol=1e-10
    end
    ...
    @testset "MRP2EP - Round Trip" begin
        ...
        β_orig = SVector{4,Float64}(cos(θ/2), sin(θ/2)*axis[1], sin(θ/2)*axis[2], sin(θ/2)*axis[3])
        
        σ = AstroCoords.EP2MRP(β_orig)
        β_back = AstroCoords.MRP2EP(σ)
        
        # Should recover original EP (within tolerance)
        @test β_back[1] ≈ β_orig[1] atol=1e-10
        @test β_back[2] ≈ β_orig[2] atol=1e-10
        @test β_back[3] ≈ β_orig[3] atol=1e-10
        @test β_back[4] ≈ β_orig[4] atol=1e-10
    end
    ...
    @testset "MRP2EP - Multiple Round Trips" begin
        ...
        for _ in 1:10
            ...
            @test β_back[1] ≈ β_orig[1] atol=1e-9
            @test β_back[2] ≈ β_orig[2] atol=1e-9
            @test β_back[3] ≈ β_orig[3] atol=1e-9
            @test β_back[4] ≈ β_orig[4] atol=1e-9
        end
    end
